...
INSERT INTO player_points (player_id, matchday_id, points, goals, assists, minutes_played, yellow_cards, red_cards, clean_sheet) VALUES
    -- Matchday 1 (id=1) — past, used for points demo
    ...
    -- Matchday 2 (id=2) — past, used for points demo
...
SELECT listing_id, user_id, amount, status FROM (VALUES
    -- Bids on Alisson (listing 1)
    (1::bigint, 1::bigint, 11000000, 'active'::bid_status),
    (1,         3,         12000000, 'active'),
    (1,         4,         10500000, 'active'),
    ...
    (4, 2, 39000000, 'active'),
    (4, 4, 42000000, 'active')
) AS v(listing_id, user_id, amount, status)
WHERE NOT EXISTS (
    SELECT 1 FROM bids b
    WHERE b.listing_id = v.listing_id AND b.user_id = v.user_id
);
